...
def validateStructPIROUTPOST(tran, source, target, tech):
	player = tran.db[source.owner]
	if target.plStratRes in (SR_TL3A, SR_TL3B, SR_TL3C):
		mod = Rules.pirateTL3StratResColonyCostMod
	else:
		mod = 1.0
...
def finishStructPIROUTPOST(tran, source, target, tech):
	...
	if source.type == T_FLEET:
		if target.plStratRes in (SR_TL3A, SR_TL3B, SR_TL3C, SR_TL5A, SR_TL5B, SR_TL5C):
			mod = Rules.pirateTL3StratResColonyCostMod
		else:
			mod = 1.0
		log.debug(source.owner, "DEPLOYING MODULE -- BEFORE", player.pirateFame)
		if target.owner != player.oid:
			famePenalty = int(mod * Rules.pirateColonyCostMod * len(player.planets))
		else:
			log.debug(source.owner, "FAME NOT MODIFIED, PLANET BELONGS TO PIRATE ALREADY")
		log.debug(source.owner, "DEPLOYING MODULE -- AFTER", player.pirateFame)
